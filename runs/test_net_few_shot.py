...
def test_epoch(val_loader, model, val_meter, cur_epoch, cfg, writer=None):
    ...
    try:
        with logging_redirect_tqdm(), tqdm.tqdm(val_loader) as pbar:
            for cur_iter, task_dict in enumerate(pbar):
                ...
                if cfg.DETECTION.ENABLE:
                    ...
                    if isinstance(task_dict['target_labels'], dict):
                        ...
                        _, top_max_k_inds = torch.topk(preds, 1, dim=1, largest=True, sorted=True)
                        for index, score in enumerate(top_max_k_inds):
                            k = str(task_dict['real_target_labels'][index].cpu().item())
                            num_per_class[k] = num_per_class.get(k, 0) + 1
                            top1_per_class[k] = top1_per_class.get(k, 0) + (score[0] == labels[index])
        ...
        pd.DataFrame([
            {
                'class': perclass, 
                'acc': (top1_per_class[perclass]/num_per_class[perclass]).item(),
                'n': num_per_class[perclass],
            } for perclass in top1_per_class
        ]).to_csv(os.path.join(cfg.OUTPUT_DIR, 'accuracy.csv'), index=False)

        all_preds = [pred.cpu().clone().detach() for pred in val_meter.all_preds]
        all_labels = [
            label.cpu().clone().detach() for label in val_meter.all_labels
        ]
        preds_np = [x.numpy() for x in all_preds]
        labels_np = [x.numpy() for x in all_labels]
        np.savez(os.path.join(cfg.OUTPUT_DIR, 'predictions.npz'), preds=preds_np, labels=labels_np)
        # cm = confusion_matrix(all_preds, all_labels)
        # np.savez(os.path.join(cfg.OUTPUT_DIR, 'confusion_matrix.npy'), cm)
